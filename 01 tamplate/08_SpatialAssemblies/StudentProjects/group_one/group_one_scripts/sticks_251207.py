...
def calculate_z_vector_from_centerline_with_z_vector(centerline_vector, frame):
    c = frame.zaxis
    x_axis = frame.xaxis
    y_axis = frame.yaxis
    angle = angle_vectors(c, centerline_vector)
...
class Stick:
    ...
    def __init__(self, axis, frame = None, width = None, depth = None):
    ...
    def _get_stick_frame(self):
        if self.frame:
            ...
            normal = calculate_z_vector_from_centerline_with_z_vector(self.axis.direction, self.frame)
        else:
